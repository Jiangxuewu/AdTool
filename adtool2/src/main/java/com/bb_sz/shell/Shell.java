...
public class Shell {
    ...
    public static String shelledOutApk;

    public static void run(String apk, String outApk) {
        shelledOutApk = outApk;
    ...
    private static void main(String curApkPath) {
        ...
        if (debug)
            System.err.print("0 outPath:" + outPath + "->" + System.currentTimeMillis() + "\n");
        File smail = new File(outPath + File.separator + "smali");
        while (!smail.exists()) {
            if (debug)
                System.err.print("not smali and waiting:" + smail.getAbsolutePath() + "->" + System.currentTimeMillis() + "\n");
            try {
                Thread.sleep(1000);
        ...
        if (debug)
            System.err.print("1 smali:" + smail.getAbsolutePath() + "->" + System.currentTimeMillis() + "\n");
        while (smail.exists()) {
        ...
        if (debug)
            System.err.print("start apk, shellApkPath:" + shellApkPath + "->" + System.currentTimeMillis() + "\n");
        ApkTools.apk(outPath, shellApkPath);
        if (debug)
            System.err.print("start check, shellApkPath:" + shellApkPath + "->" + System.currentTimeMillis() + "\n");
